import { ROOTS, SCALE_ROOT_IDS } from "../data.js?v=4";
import { rootById, buildDiatonicChords, buildChordTones } from "../theory.js?v=5";
...
// Two entry points share this one quiz — Lesson 8 (where the theory lives) and the Practicar tab
// (so practicing it doesn't require leaving that tab). Both read/write the same module-level state
// below, so progress and mastery are unified regardless of which surface was used last.
const FUNCTION_QUIZ_ROOTS=["C","G","F"];
let functionQuizChord=null, functionQuizAnswered=false, functionMissionCorrect=0;
function initFunctionQuiz(ids){
  function renderFunctionOptions(){
    const c=document.getElementById(ids.options); c.innerHTML="";
  ...
  function newFunctionChallenge(){
    ...
    document.getElementById(ids.feedback).className="result-box";
    document.getElementById(ids.feedback).textContent="Puedes repetir antes de responder.";
    document.getElementById(ids.repeat).disabled=false;
  }
  function answerFunctionQuiz(answer){
    const box=document.getElementById(ids.feedback);
    ...
    if(correct){
      ...
      renderMissionDots(document.getElementById(ids.missionDots),[0,1,2].map(i=>i<Math.min(functionMissionCorrect,3)));
      if(functionMissionCorrect>=3){
        setLessonState(8,"mastered");
        document.getElementById(ids.missionText).innerHTML="<strong>¡Dominado!</strong> Reconoces la función armónica de tónica, subdominante y dominante.";
      }
    }
  }
  document.getElementById(ids.newChallenge).addEventListener("click",newFunctionChallenge);
  document.getElementById(ids.repeat).addEventListener("click",()=>functionQuizChord&&playChord(functionQuizChord.tones.map(t=>t.midi)));
  renderFunctionOptions();
}
initFunctionQuiz({options:"functionOptions",feedback:"functionFeedback",repeat:"functionRepeat",newChallenge:"functionNewChallenge",missionDots:"functionMissionDots",missionText:"functionMissionText"});
initFunctionQuiz({options:"practiceFunctionOptions",feedback:"practiceFunctionFeedback",repeat:"practiceFunctionRepeat",newChallenge:"practiceFunctionNewChallenge",missionDots:"practiceFunctionMissionDots",missionText:"practiceFunctionMissionText"});
