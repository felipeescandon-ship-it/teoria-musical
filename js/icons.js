...
// Renders mission-progress steps as real circles (CSS-styled .mission-dot elements) instead of
// the ○/● unicode characters previously used — `flags` is an array of booleans, one per step.
export function renderMissionDots(el, flags) {
  el.innerHTML = flags.map(f => `<span class="mission-dot${f ? " filled" : ""}"></span>`).join("");
}
